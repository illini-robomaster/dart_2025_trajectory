...
```
dart_vision/
├── dart_detector.py              # 主程序：飞镖检测与轨迹追踪（带显示窗口）
├── dart_detector_headless.py     # 无界面版本：性能测试用（英文输出）
...
### 预期性能
- **实时FPS（带检测）**：~40 FPS @ 640x480
- **纯采集FPS**：~80 FPS @ 640x480
- **启动时间**：<5秒
- **检测延迟**：<50ms

### 性能瓶颈解决
- ✅ **自动曝光问题**：关闭AE，使用手动曝光20ms（关键优化！）
- ✅ **分辨率优化**：640x480替代1280x1024
- ✅ **算法简化**：减少形态学操作次数
...
### v2.1 (2026-01-21) - 性能优化
- 🚀 **重大性能提升**：从13 FPS → 40+ FPS
- ✅ 发现并解决自动曝光限制帧率问题（关键！）
- ✅ 优化相机分辨率至640x480
- ✅ 设置手动曝光20ms
- ✅ 自动缩放起始点坐标适配新分辨率
- ✅ 添加dart_detector_headless.py性能测试版本
- ✅ headless版本英文输出
